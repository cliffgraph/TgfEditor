...
bool t_FileExist(const tstring& targetPath)
{
#ifdef _WIN32
	BOOL find = PathFileExists(targetPath.c_str());
...
const tstring &t_GetModuleFileName(tstring *pName)
{
#ifdef _WIN32
...
static tstring &getVerQueryValue(const uint8_t *pBlock, const TCHAR *pTerget, tstring*pResultStr)
{
	// pTerget:
	...
	UINT queryLen;
	uint16_t *pLangCode;
	VerQueryValue(pBlock, _T( "\\VarFileInfo\\Translation" ), (void**)&pLangCode, &queryLen);
	tostringstream oss;
	oss	<< _T("\\StringFileInfo\\")
		<< std::hex << std::setw(4) << std::setfill(_T('0')) << pLangCode[0]
		<< std::hex << std::setw(4) << std::setfill(_T('0')) << pLangCode[1]
		<< _T("\\") << pTerget;
	TCHAR *pStr;
	VerQueryValue(pBlock, oss.str().c_str(), (void**)&pStr, &queryLen );
	*pResultStr = (0 < queryLen) ? pStr : _T("");
	return *pResultStr;
}


bool t_GetFileVersionInfo(
	const tstring &tergetPath,
	tstring *pNameStr, tstring *pVerNumberStr, tstring *pCopyrightStr)
{
	bool bRetc = false;
#ifdef _WIN32
	uint32_t size = GetFileVersionInfoSize(tergetPath.c_str(), nullptr);
	std::unique_ptr<uint8_t[]> pVer(NNEW uint8_t[size]);
	if( GetFileVersionInfo(tergetPath.c_str(), 0, size, pVer.get()) ){
		// Version number
		VS_FIXEDFILEINFO* pFileInfo;
		UINT queryLen;
		if( pVerNumberStr != nullptr ){
			VerQueryValue(pVer.get(), _T( "\\" ), (void**)&pFileInfo, &queryLen);
			tostringstream oss;
			oss
				<< HIWORD(pFileInfo->dwFileVersionMS) << _T(".")
				<< LOWORD(pFileInfo->dwFileVersionMS) << _T(".")
				<< HIWORD(pFileInfo->dwFileVersionLS) << _T(".")
				<< LOWORD(pFileInfo->dwFileVersionLS);
			*pVerNumberStr = oss.str();
		}
		if( pCopyrightStr != nullptr ){
			getVerQueryValue(pVer.get(), _T("LegalCopyright"), pCopyrightStr);
		}
		if( pNameStr != nullptr ){
			getVerQueryValue(pVer.get(), _T("InternalName"), pNameStr);
		}
		bRetc = true;
	}
#endif
